...
lighttime_truth = load('lighttime_truth-1.mat');
lighttime_truth = cell2mat(struct2cell(lighttime_truth));
LEO_data = load('LEO_DATA_Apparent-1.mat');
LEO_data = cell2mat(struct2cell(LEO_data));
...
dUT1 = 0.1964940; % [seconds]
% Polar motion values hardcoded from Vallado. 
pm = [-0.140682 0.333309]./3600; % [degrees]
...
% Convert the tracking station position from ECEF to ECI.
atollECI = ecef2eci(UTC, atoll, 'dAT', delAT, 'dUT1', dUT1, 'pm', pm);
diegoECI = ecef2eci(UTC, diego, 'dAT', delAT, 'dUT1', dUT1, 'pm', pm);
areciboECI = ecef2eci(UTC, arecibo, 'dAT', delAT, 'dUT1', dUT1, 'pm', pm);
...
% Substitute numerical values into symbolic expressions
dUdx_num = subs(dUdx, {'J2', 'muu', 'RE', 'C_D', 'A', 'm', 'rho0', 'r0', ...
    'H', 'om_E', 'mu_Sun', 'mu_Moon'}, {J2, mu, RE, C_D, AreaX, m, rho0, ...
    r0, H, om_E, mu_Sun, mu_Moon});
dUdy_num = subs(dUdy, {'J2', 'muu', 'RE', 'C_D', 'A', 'm', 'rho0', 'r0', ...
    'H', 'om_E', 'mu_Sun', 'mu_Moon'}, {J2, mu, RE, C_D, AreaX, m, rho0, ...
    r0, H, om_E, mu_Sun, mu_Moon});
dUdz_num = subs(dUdz, {'J2', 'muu', 'RE', 'C_D', 'A', 'm', 'rho0', 'r0', ...
    'H', 'om_E', 'mu_Sun', 'mu_Moon'}, {J2, mu, RE, C_D, AreaX, m, rho0, ...
    r0, H, om_E, mu_Sun, mu_Moon});
...
[T,Y] = ode45(@propStateAndSTM, times, XSTM0, myoptions, AMat_func, ...
    dUdx_func, dUdy_func, dUdz_func, interp_rsun, interp_rmoon); 
...
relDiffSTM = (STMt(:,:,end) - STM_21600_0)./STM_21600_0

figure(2)
histogram(reshape(log10(abs(relDiffSTM)),49,1), 10)
xlabel("Log of the Relative Difference")
ylabel("Frequency")
title("Histogram of the Log of Relative Differences in STM Components")


%% Get measurements
% Find matching time indices
[~, idx] = ismember(LEO_data(:,2), T);

% Filter B to keep only matching rows
Xt_filtered = Xt(idx(idx > 0), :);
% scatter3(Xt_filtered(:,1), Xt_filtered(:,2), Xt_filtered(:,3))

ranges = [];
rangerates = [];
for k = 1:length(LEO_data)
    station = LEO_data(k,1);
    UTCplus = UTC + datevec(seconds(LEO_data(k,2)));

    if station == 1
        gs = ecef2eci(UTCplus, atoll, 'dAT', delAT, 'dUT1', dUT1, 'pm', pm);
    elseif station == 2
        gs = ecef2eci(UTCplus, diego, 'dAT', delAT, 'dUT1', dUT1, 'pm', pm);
    else
        gs = ecef2eci(UTCplus, arecibo, 'dAT', delAT, 'dUT1', dUT1, 'pm', pm);
    end

    range = norm(Xt_filtered(k,1:3)' - gs);
    rangerate = (Xt_filtered(k,1:3)' - gs).'*(Xt_filtered(k,4:6)')/range;
    ranges = [ranges; range];
    rangerates = [rangerates; rangerate];
end

% Compute the RMS Errors
RMSrange = sqrt(sum(LEO_data(:,3) - ranges)^2/length(ranges))
RMSrangerate = sqrt(sum(LEO_data(:,4) - rangerates)^2/length(ranges))

figure(3)
scatter(LEO_data(:,2), ranges)
hold on
grid on
scatter(LEO_data(:,2), LEO_data(:,3))
hold off

figure(4)
scatter(LEO_data(:,2), rangerates)
hold on
grid on
scatter(LEO_data(:,2), LEO_data(:,4))
hold off

figure(5)
scatter(LEO_data(:,2), ranges - LEO_data(:,3))
title('Range Measurement Residuals over Time')
xlabel('Time [s]')
ylabel('Range Error [km]')

figure(6)
scatter(LEO_data(:,2), rangerates - LEO_data(:,4))
title('Range-Rate Measurement Residuals over Time')
xlabel('Time [s]')
