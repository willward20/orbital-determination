...
ts1_sigmas = [(10/1000); 0.5*(1e-4)];
ts2_sigmas = [(5/1000); 1*(1e-6)];
...
% Prepare for EKF
% Unpack the LEO Data
LEO_data = LEO_data(1:435, :);
LEO_data = LEO_data(LEO_data(:,1) == 3, :);
...
% Mats for recording.
xHatMat = zeros(6, len); % 6 states (don't estimate C_D) 
PkMat = zeros(6,6,len); % covariance
delzks = zeros(2, len);
...
xSTMk = [R0; V0; C_D; reshape(eye(7), 7^2, 1)];

% Test #1: Propagate to 24 hours.
load('truth.mat')

% next_times = 0:60:sec_per_day; % [s]
% 
% % Propagate the kinematics forward in time.
% [T2,Y2] = ode45(@propStateAndSTM, next_times, xSTMk, myoptions, AMat_func, ...
%     dUdx_func, dUdy_func, dUdz_func, rsunMat, rmoonMat);
% 
% % Extract the components of Ystar
% Xt_final = Y2(end,1:6)';
% 
% AbsErr = abs((Xt_final - truth))

% Add the end time.
tVecM = [tVecM; sec_per_day];
...
for k = 1:len+1
    ...
    if tVecM(k) == 0
        ...
        % Propagate the kinematics forward in time.
        if k == 1
            [~,Y] = ode45(@propStateAndSTM, [0:60:tVecM(k)]', xSTMk, myoptions, AMat_func, ...
                dUdx_func, dUdy_func, dUdz_func, rsunMat, rmoonMat); 
        else
            [~,Y] = ode45(@propStateAndSTM, tVecM(k-1:k), xSTMk, myoptions, AMat_func, ...
                dUdx_func, dUdy_func, dUdz_func, rsunMat, rmoonMat); 
        end
    ...
    % disp('Rank of Pk')
    % disp(eig(Pk)')
    % disp(eig(scriptHk*PkBar*scriptHk' + R))
...
ward_pos_case = Xt_final(end,1:3)';
ward_cov_case = cov(1:3,1:3,end);
% save('caseF.mat', "ward_pos_case", "ward_cov_case")
...
figure(7)
scatter(tVecM, postfit_delzks(:,1))
hold on
grid on
plot(tVecM, postfit_delzks(:,1) - 3*sigMat(:,1), 'k', 'LineStyle','--','LineWidth',1.5)
plot(tVecM, postfit_delzks(:,1) + 3*sigMat(:,1), 'k', 'LineStyle','--','LineWidth',1.5)
title('Range Measurement Postfit Residuals over Time')
xlabel('Time [s]')
ylabel('Range Error [km]')

figure(8)
scatter(tVecM, postfit_delzks(:,2))
hold on
grid on
plot(tVecM, postfit_delzks(:,2) - 3*sigMat(:,2), 'k', 'LineStyle','--','LineWidth',1.5)
plot(tVecM, postfit_delzks(:,2) + 3*sigMat(:,2), 'k', 'LineStyle','--','LineWidth',1.5)
title('Range-Rate Measurement Postfit Residuals over Time')
xlabel('Time [s]')
