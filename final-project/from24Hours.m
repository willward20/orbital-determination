...
ts1_sigmas = [(10/1000); 0.5*(1e-6)];
ts2_sigmas = [(5/1000); 1*(1e-6)];
ts3_sigmas = [(10/1000); 0.5*(1e-6)];
...
% Unpack truth at 24 hours. 
load('truth.mat')

% The prior state PDF (initial condition and covar)
xBar = [truth(1:3); truth(4:6)]; %[R0; V0];
PBar = diag([0.1 0.1 0.1 0.01 0.01 0.01]);
...
xSTMk = [truth(1:3); truth(4:6); C_D; reshape(eye(7), 7^2, 1)];

next_times = sec_per_day:60:7*sec_per_day; % [s]
...
AbsErr = abs(Xt_final(1:3) - Jack)


% Add 24 hours to beginning of time vec.
% tVecM = [sec_per_day; tVecM];
% Add the end time.
...
for k = 1:len+1
    ...
    if tVecM(k) == sec_per_day % 0
        ...
        % Propagate the kinematics forward in time.
        if k == 1
            [T,Y] = ode45(@propStateAndSTM, [sec_per_day:60:tVecM(k)]', xSTMk, myoptions, AMat_func, ...
                dUdx_func, dUdy_func, dUdz_func, rsunMat, rmoonMat); 
        else
...
figure(7)
scatter(tVecM(1:len), postfit_delzks(:,1))
hold on
grid on
...
figure(8)
scatter(tVecM(1:len), postfit_delzks(:,2))
hold on
grid on
